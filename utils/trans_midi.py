...
def midi_to_json(midi_file_path: str, output_json_path: str, file_name: str):
    # 加载MIDI文件
    ...
    for track in mid.tracks:
        ...
        for msg in track:
            ...
            if msg.type == "set_tempo":
                ...
                events.append({"type": "bpm", "beat": beat, "bpm": bpm})
                current_bpm = bpm
    ...
    for track_idx, track in enumerate(mid.tracks):
        ...
        for msg in track:
            ...
            if (
                msg.type == "note_on"
                and msg.velocity > 0
                and NOTE_MIN <= msg.note <= NOTE_MAX
            ):
                # 截断相同音符重叠的部分
                if msg.note in active_notes:
                    ...
                    for note_info in active_notes[msg.note][:]:
                        ...
                        if (
                            note_info["end_beat"] is None
                            or current_beat < note_info["end_beat"]
                        ):
                            # 截断现有音符
            ...
            if (
                ...
                if note_solfege := midi_note_to_solfege(msg.note):
                    if duration > threshold_ticks:
                        ...
                        events.append(
                            {"type": "tap", "beat": start_beat, "note": note_solfege}
                        )
...
if __name__ == "__main__":
    for input_midi in os.listdir("./midi"):
        file_name = os.path.basename(input_midi)
        output_json = f"score/{file_name.removesuffix(".mid")}.json"  # 输出JSON文件路径

        # 转换MIDI文件
        events = midi_to_json(
            os.path.join("./midi", input_midi),
            output_json,
            file_name.removesuffix(".mid"),
        )
